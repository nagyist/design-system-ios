...
const CONFIG = {
  ios_color_factory_template:'SLDSColorInptTemplate',
  ios_color_factory:'SLDSColorInpt',
  colors_file:'input.json'
};
...
gulp.task('input', () => {
  fs.writeFile(path.join(__PATHS__.temp, 'input.json'), '{"tempDir":'+'"'+__PATHS__.generated+'"}');
  return gulp.src(path.join(__PATHS__.temp, 'input.json'))
    .pipe(readColors(colorsFile, true, (name) => { return camelCase(name).replace('color','SLDSColorInput') }))
    .pipe(readTemplate(templateFile + '.h', 'headerSrc'))
    .pipe(readTemplate(templateFile + '.m', 'classSrc'))
    .pipe(replaceHeaderTokens(CONFIG.ios_color_factory_template, CONFIG.ios_color_factory, 'SLDS_COLOR_INPUT_NOTFOUND,'))
