...
const CONFIG = {
  ios_color_factory_template:'SLDSColorBtnTemplate',
  ios_color_factory:'SLDSColorBtn',
  colors_file:'button.json'
};
...
export const replaceClassTokens = (templateName, factoryName) => {
    ...
    let colorCases = options.colorEnum.map(function(name){
      let body = [];
...
gulp.task('button', () => {
  fs.writeFile(path.join(__PATHS__.temp, 'button.json'), '{"tempDir":'+'"'+__PATHS__.generated+'"}');
  return gulp.src(path.join(__PATHS__.temp, 'button.json'))
    .pipe(readColors(colorsFile, true, (name) => { return camelCase(name).replace('color','SLDSColorBtn') }))
    .pipe(readTemplate(templateFile + '.h', 'headerSrc'))
    .pipe(readTemplate(templateFile + '.m', 'classSrc'))
    .pipe(replaceHeaderTokens(CONFIG.ios_color_factory_template, CONFIG.ios_color_factory, 'SLDS_COLOR_BTN_NOTFOUND,'))
