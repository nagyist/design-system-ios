...
const CONFIG = {
  ios_color_factory_template:'SLDSColorMsgTemplate',
  ios_color_factory:'SLDSColorMsg',
  colors_file:'messaging.json'
};
...
gulp.task('messaging', () => {
  fs.writeFile(path.join(__PATHS__.temp, 'messaging.json'), '{"tempDir":'+'"'+__PATHS__.generated+'"}');
  return gulp.src(path.join(__PATHS__.temp, 'messaging.json'))
    .pipe(readColors(colorsFile, true, (name) => { return camelCase(name).replace('color','SLDSColorMsg') }))
    .pipe(readTemplate(templateFile + '.h', 'headerSrc'))
    .pipe(readTemplate(templateFile + '.m', 'classSrc'))
    .pipe(replaceHeaderTokens(CONFIG.ios_color_factory_template, CONFIG.ios_color_factory, 'SLDS_COLOR_MSG_NOTFOUND,'))
