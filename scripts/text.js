...
const CONFIG = {
  ios_color_factory_template:'SLDSColorTxtTemplate',
  ios_color_factory:'SLDSColorTxt',
  colors_file:'text.json'
};
...
gulp.task('text', () => {
  fs.writeFile(path.join(__PATHS__.temp, 'text.json'), '{"tempDir":'+'"'+__PATHS__.generated+'"}');
  return gulp.src(path.join(__PATHS__.temp, 'text.json'))
    .pipe(readColors(colorsFile, false))
    .pipe(readTemplate(templateFile + '.h', 'headerSrc'))
    .pipe(readTemplate(templateFile + '.m', 'classSrc'))
    .pipe(replaceHeaderTokens(CONFIG.ios_color_factory_template, CONFIG.ios_color_factory, 'SLDS_COLOR_TXT_NOTFOUND,'))
